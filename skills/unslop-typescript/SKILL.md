...
The rule names in the Enforceable-by column come from the `anti-slop` Oxlint plugin, not
from this repo. **None of them is wired up here yet.** Every criterion in this skill is
review-only today, which means it holds while an agent applies it and not otherwise.

The column exists so the split is visible. Roughly half of these criteria are
mechanically decidable and half need judgment. When the lint layer lands, the enforceable
half stops depending on anyone remembering.
