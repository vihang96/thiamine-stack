...
Don't invent jargon. Use the words a developer would say out loud: "move", "delete", "a budget that only decreases", not "evacuate", "ratchet", or "endgame". A named pattern is fine when the doc says what it means the first time. Add new offenders to `unslop-prose`'s abstract-metaphor rule with their replacement.

## Scope

This skill owns document-level decisions: which mode a document is, how it is
structured, what its headings say, and how voice changes with mode.

The `unslop-prose` skill owns sentence-level AI tells: word choice, punctuation, filler,
hedging, and formatting artifacts. Apply it to every doc this skill touches.

The two overlap on punctuation, sentence length, passive voice, and plain word choice.
Both say the same thing there, and the layers below keep those rules because the
standards they cite include them. Where the two disagree, this skill wins. Mode decides
voice, so unslop-prose's advice to have opinions, use "I", and let some mess in belongs to
explanation and to prose with a voice, not to reference or how-to.
...
- Apply the **unslop-prose** skill to every doc this skill touches. Scope above says which skill wins where the two overlap.
