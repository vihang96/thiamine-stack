...
2. State the intent in one paragraph, and say where you got it: the description, the commit
...
3. Judge reviewability before content. Is it one concern, or a refactor and a feature
...
4. Run the harness's diff review if it has one. In Claude Code that is the `code-review`
...
5. Add the four things it does not do.

   - **Intent.** Does the change achieve step 2, and does it do the thing it claims?
...
6. Verify before reporting, and before dismissing. Read the enclosing function, grep the
...
7. Rank, cut, and deliver per `playbooks/deliver-the-review.md`.
