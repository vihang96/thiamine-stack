...
1. **Is it real?** A signal can be an artifact of its own collection: a monitor with the
   ...
   crawler. Confirm it happened to a person or to real data before spending another
...
2. **Is it new?** Search the tracker for the fingerprint and for the symptom in the words
   ...
   it and file nothing. A closed item that has come back is worth more than a new one. The
   last fix is now evidence, so file it as a regression and name the commit that closed it.
...
4. **What does it cost to leave for a month?** This is the ranking question. It replaces guessing at
   severity. Say it in the units the item has: users affected, money settled
