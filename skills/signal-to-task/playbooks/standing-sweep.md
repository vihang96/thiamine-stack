...
2. Pull what each kind is good for. The trap column is the one that matters, because each
   ...
   | CI | flakes by rate, and jobs failing on a branch nobody is carrying a change on | the test id plus the assertion | a flake is a signal about the test, and a real one about the code. Do not merge them. A red check on your own change is `drive-ci-green`, not this |
