...
Adapted from a Cursor continual-learning workflow, which drove the loop from a Stop hook and
wrote its findings into `AGENTS.md`. Both were changed here: the hook suggests rather than
interrupts, and the findings go to the harness memory store, because in a thiamine install
